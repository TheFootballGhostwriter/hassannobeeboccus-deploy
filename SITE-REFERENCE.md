...
**Guide covers are a deliberate exception to the split, and are not to be "fixed".** The table above was derived from a census of the main site pages. On the three document covers, terracotta legitimately does surface work: the 4px `.cover::before` bar and the 48px `.cover-divider` on `alternative-models-framework.html` and `the-coaching-pathway-guide-in-england.html` are both terracotta, alongside a terracotta `.cover-eyebrow`. That is coherent on a near-black or dark-green cover and does not read as generated — a cover is a single composed object, not a page of running text competing for emphasis. Two related things follow from it. First, `alternative-models-framework.html:470` scopes `--terracotta: #e05a4c` to `.cover, .section-divider-page, .cta-page` so terracotta on dark backgrounds picks up the brighter dark-mode value; that duplicate definition is the documented light/dark pair, not a bug. Second, the cover bar on `conversation-to-content-map.html` is **gold** rather than terracotta because that cover's background is itself terracotta. The three cover bars differing is a consequence of three different cover backgrounds, not drift. Checked and left as-is 03/09/2026.
...
- **Numerals**: any number that animates or updates in place needs `font-variant-numeric: tabular-nums`. Proportional digits are different widths (`1` is narrow, `8` is wide), so a count-up stat physically grows and shrinks as it climbs. Measured on the homepage before the fix: a 17px width swing between values of identical digit length. Applies to `.proof-stat .stat-value` and `.proof-engagement .stat-value`, both of which count up. Static numbers inside sentences do **not** need this and should keep proportional digits.
- **Count-up suffixes** (`+`, `%`): the counter script overwrites `textContent` on `.proof-value` from its first frame, so a suffix cannot live inside that span. Put it in a sibling `<span class="stat-value">` carrying no `data-target`, and wrap number and suffix together in a single element (`.stat-figure` on the homepage). The wrapper is load-bearing: `.proof-stat` is a flex container, and as bare children the two spans get blockified and separated. Keep the real comma-formatted number as static content inside `.proof-value` — link previews, scrapers and screenshots read the HTML, and a hardcoded `0` is how the proof section once shared as three zeros.
- **Gold is a dark-mode-only text colour.** `--gold` (#C9A84C) scores 2.29:1 on the white light-mode card, well under the 4.5:1 AA floor for text under 18.66px. Stat numbers pair `color: var(--green)` with a `body.dark` override to `var(--gold)` for this reason. Never set gold text without the light-mode counterpart.
- **Proof figures are floors**, rounded down to the nearest thousand (`75,000+`, not `75,283`). Exact figures went stale twice in six months. A floor never becomes wrong by the number rising, so do not "correct" one to the live value. The same applies to the aggregate in the caption below the cards.
